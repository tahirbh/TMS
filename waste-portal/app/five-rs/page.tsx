...
const ReduceSvg = () => (
  <svg viewBox="0 0 120 120" className="w-full h-full" fill="none">
    ...
    <path d="M28 60 L42 60" stroke="#00C47D" strokeWidth="2.5" strokeLinecap="round" />
...
function RSection({ r, index }: { r: R; index: number }) {
  ...
  return (
    <section ref={ref} className="relative min-h-screen flex items-center overflow-hidden py-24"
      style={{ background: index % 2 === 0 ? '#ffffff' : 'var(--bg-secondary)' }}>

      {/* Animated background orb */}
      <motion.div style={{ y: bgY }}
        className="absolute w-[800px] h-[800px] rounded-full pointer-events-none"
        style={{
          background: `radial-gradient(circle, ${r.color}10, transparent 65%)`,
      ...
      <div className="container-xl relative z-10">
        <div className={`grid lg:grid-cols-2 gap-16 xl:gap-24 items-center ${isEven ? '' : 'direction-rtl'}`}>
          ...
          <motion.div
            ...
            <h2 className="heading-xl text-slate-800 mb-3">{r.name}</h2>
            <p className="text-lg font-semibold mb-6" style={{ color: r.color }}>{r.tagline}</p>

            <p className="text-slate-600 leading-relaxed mb-4 text-base">{r.desc1}</p>
            <p className="text-slate-500 leading-relaxed mb-8 text-base">{r.desc2}</p>
          ...
          <motion.div
            ...
            <motion.div
              ...
              <div className="absolute inset-0 rounded-full"
                style={{ background: `radial-gradient(circle, ${r.color}30, transparent 70%)`, filter: 'blur(20px)' }} />
              <div className="relative w-full h-full rounded-3xl flex items-center justify-center p-6 glass-card"
                style={{ background: 'white', border: `2px solid ${r.color}30`, backdropFilter: 'blur(10px)' }}>
                <r.SvgIcon />
              </div>
            </motion.div>
            ...
            <div className="space-y-4">
              <h4 className="text-slate-400 text-xs uppercase tracking-widest font-semibold">Real-World Examples</h4>
              {r.examples.map((ex, ei) => (
                <motion.div key={ei}
                  ...
                  className="flex items-start gap-4 p-4 rounded-2xl cursor-default glass-card"
                  style={{ background: 'white', border: `1px solid ${r.color}20` }}>
                  <span className="text-2xl flex-shrink-0 mt-0.5">{ex.emoji}</span>
                  <div>
                    <h5 className="font-bold text-slate-800 text-sm mb-1">{ex.title}</h5>
                    <p className="text-slate-500 text-xs leading-relaxed">{ex.desc}</p>
                  </div>
      ...
      <section className="relative min-h-[70vh] flex items-center justify-center overflow-hidden"
        ...
        <div className="relative z-10 text-center container-lg">
          ...
          <motion.p initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.25 }}
            className="text-slate-600 text-body-lg max-w-2xl mx-auto mb-12">
      ...
      <section className="py-20" style={{ background: 'var(--bg-primary)' }}>
        <div className="container-lg">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
            ...
            <h2 className="heading-lg text-slate-800 mb-3">The Waste Hierarchy</h2>
            <p className="text-slate-500 text-base">Most to least impactful — always act as high up the pyramid as possible</p>
          </motion.div>
          ...
          <div className="max-w-lg mx-auto space-y-2">
            {rsData.map((r, i) => (
              <motion.div key={i}
                ...
                className="flex items-center gap-4 px-6 py-3 rounded-xl cursor-pointer glass-card animate-pulse"
                style={{
                  background: 'white',
                ...
                <span className="text-slate-500 text-xs ml-auto">{r.tagline}</span>
      ...
      <section className="section-padding" style={{ background: 'var(--gradient-hero)' }}>
        <div className="container-lg text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }}
            ...
            <h2 className="heading-xl text-slate-800 mb-4">Every Action <span className="gradient-text">Matters</span></h2>
            <p className="text-slate-600 text-body-lg max-w-2xl mx-auto mb-10">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              <Link href="/recycling-process" className="btn-crystal btn-crystal-primary flex items-center gap-2">
                <Recycle className="w-5 h-5" /> Learn the Recycling Process
              </Link>
              <Link href="/waste-categories" className="btn-crystal btn-crystal-glass text-slate-700 border-slate-300">
